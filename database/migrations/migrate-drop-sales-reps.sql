-- ============================================
-- MIGRATION: Drop sales_reps table and update all FKs to auth.users
-- ============================================
--
-- PREREQUISITES:
-- 1. Run audit-sales-reps-usage.sql first to check for orphaned data
-- 2. Backup your database
-- 3. Read through this entire script before running
--
-- WHAT THIS DOES:
-- - Removes all foreign key constraints pointing to sales_reps
-- - Updates constraints to point to auth.users instead
-- - Drops the legacy sales_reps table
-- - Fixes the "Unknown" user issue
--
-- ============================================

-- START TRANSACTION (uncomment when ready to run)
-- BEGIN;
...
BEGIN
  ...
  RAISE NOTICE '  • Nullified orphaned user references';
...
-- COMMIT TRANSACTION (uncomment when ready to run)
-- COMMIT;
