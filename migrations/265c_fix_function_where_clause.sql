-- ============================================
-- Migration 265c: Fix compute_api_opportunities() function
-- ============================================
-- The function needs WHERE clauses on all UPDATE statements
-- to comply with database safety settings.
...
BEGIN
    ...
    -- Update job linkage (WHERE clause required by database settings)
    ...
    -- Compute derived fields (WHERE id IS NOT NULL to satisfy safety check)
    UPDATE jobber_api_opportunities o
    SET
...
SELECT 'compute_api_opportunities function updated successfully' AS status;
