-- ============================================
-- Migration 275: Fix request linkage and salesperson source
-- ============================================
-- Two bugs:
-- 1. Salesperson was pulled from quotes (always null) instead of requests
-- 2. Request linkage used quote_jobber_ids (always null in requests table)
--    Now matches via client_name + street with time proximity
-- Uses temp table for request normalization to avoid repeated string ops
...
BEGIN
    -- Step 1: Delete opportunities affected by recent sync data
    ...
    -- Step 2: Rebuild affected opportunities with 28-day windowed grouping
    ...
    INSERT INTO jobber_api_opportunities (
        ...
        quote_count, quote_numbers, quote_jobber_ids,
    ...
    -- Step 3: Link jobs (fast, uses btree + GIN indexes)
    ...
    -- Step 4: Link requests + salesperson via pre-normalized temp table
    -- Pre-compute normalized fields once to avoid repeated string ops in the join
    CREATE TEMP TABLE _req_norm ON COMMIT DROP AS
    SELECT
        jobber_id,
        salesperson,
        assessment_start_at,
    ...
    -- Match best request: most recent assessment before (or near) the first quote
    UPDATE jobber_api_opportunities o
    SET
    ...
    FROM (
        ...
        WHERE o2.last_computed_at >= NOW() - INTERVAL '1 minute'
    ...
    -- Step 5: Fallback salesperson for opps that didn't match on time window
    UPDATE jobber_api_opportunities o
    ...
    FROM (
        ...
        WHERE o2.salesperson IS NULL
          AND o2.last_computed_at >= NOW() - INTERVAL '1 minute'
          AND rn.salesperson IS NOT NULL AND rn.salesperson != ''
        ORDER BY o2.id, rn.created_at DESC
    ) fb
    WHERE o.id = fb.opp_id;

    DROP TABLE IF EXISTS _req_norm;

    -- Step 6: Compute cycle time metrics
    UPDATE jobber_api_opportunities o
    SET
        days_to_quote = CASE
