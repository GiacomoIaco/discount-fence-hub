-- ============================================
-- Migration 273: Time-windowed opportunity grouping (28-day rule)
-- ============================================
-- Previously: ALL quotes for client+street = one mega-opportunity.
-- Now: Quotes within 28 days of each other = one opportunity.
-- Same client+address with quotes 4+ weeks apart = separate opportunities.
-- Matches real business pattern: 1 request -> 1-3 quotes -> 1-2 jobs -> 1 invoice.
...
BEGIN
    ...
    -- Step 1: Insert opportunities with 28-day windowed grouping.
    -- LAG() detects gaps > 28 days between quotes for same client+street,
    -- cumulative SUM assigns group numbers. Same client+street with a gap
    -- becomes a separate opportunity.
    ...
    -- Step 2: Link jobs via quote_jobber_id (btree indexed, fast)
    ...
    -- Step 3: Link requests (GIN indexed, fast)
    UPDATE jobber_api_opportunities o
    SET
    ...
    -- Step 4: Compute cycle time metrics
    UPDATE jobber_api_opportunities o
    SET
        days_to_quote = CASE
        ...
        END
    WHERE o.assessment_date IS NOT NULL OR o.won_date IS NOT NULL;
