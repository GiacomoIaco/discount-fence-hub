-- ============================================
-- OPTIMIZED COMPUTE OPPORTUNITIES FUNCTION
-- Migration 246: Faster version that works in batches
-- ============================================
...
BEGIN
    ...
    -- Step 4: Update request/assessment linkage
    UPDATE jobber_api_opportunities o
    SET
        request_jobber_id = r.jobber_id,
        assessment_date = r.assessment_start_at::DATE
    FROM jobber_api_requests r
    WHERE r.jobber_id = ANY(
        SELECT req.jobber_id FROM jobber_api_requests req
        JOIN jobber_api_quotes q ON q.request_jobber_id = req.jobber_id
        WHERE q.jobber_id = ANY(o.quote_jobber_ids)
    )
    AND r.assessment_start_at IS NOT NULL;

    RAISE NOTICE 'Updated request linkage';
