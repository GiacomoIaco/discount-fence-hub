-- Migration: Relax task_assignees RLS policies
-- Issue: Users with function access should be able to add/remove assignees
-- Current policies only allow lead/admin roles, which is too restrictive
...
-- ============================================
-- NEW RLS POLICIES (more permissive)
-- ============================================
...
ON task_assignees FOR SELECT USING (
  EXISTS (
    ...
    AND (
      ...
      OR
      -- User has ANY function access (for non-personal, non-private initiatives)
      (
...
-- More permissive: anyone who can see the task can add assignees
...
ON task_assignees FOR INSERT WITH CHECK (
  EXISTS (
    ...
    AND (
      ...
      (pi.is_personal = true AND pi.created_by = auth.uid())
      OR
      -- User is already an assignee of this task (can add co-assignees)
      EXISTS (
        SELECT 1 FROM task_assignees ta2
        WHERE ta2.task_id = t.id
        AND ta2.user_id = auth.uid()
      ...
      OR
      -- User has function access (any role, not just lead/admin)
      (
        pi.is_personal IS NOT TRUE
        AND pi.area_id IS NOT NULL
        AND EXISTS (
...
ON task_assignees FOR DELETE USING (
  ...
  EXISTS (
    ...
    AND (
      ...
      (pi.is_personal = true AND pi.created_by = auth.uid())
      OR
      -- User has function access (any role, for non-personal initiatives)
      (
        pi.is_personal IS NOT TRUE
        AND pi.area_id IS NOT NULL
        AND EXISTS (
