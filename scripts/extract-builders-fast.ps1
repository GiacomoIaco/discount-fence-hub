...
for ($row = 5; $row -le $totalRows; $row++) {
    ...
    if ($customerName -and -not $allCustomers.ContainsKey($customerName)) {
...
Write-Host "Loaded $($allCustomers.Count) unique customers"
Write-Host ""
...
foreach ($builder in $builders) {
    if ($allCustomers.ContainsKey($builder)) {
        $info = $allCustomers[$builder]
        Write-Host "FOUND: $builder"
        $foundList += $builder
    } else {
        Write-Host "NOT FOUND: $builder"
        $notFoundList += $builder
    }
}

Write-Host ""
Write-Host "=== SUMMARY ==="
Write-Host "Found: $($foundList.Count)"
Write-Host "Not Found: $($notFoundList.Count)"
Write-Host ""
Write-Host "=== NOT FOUND LIST ==="
foreach ($name in $notFoundList) {
    Write-Host "  - $name"
}
