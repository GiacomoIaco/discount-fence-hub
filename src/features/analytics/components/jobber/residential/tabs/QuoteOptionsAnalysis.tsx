// Quote Options Analysis Tab - Win rate by number of quote options
// Shows: Win rate for 1 vs 2+ quotes per opportunity

import { FileText, TrendingUp, TrendingDown, Minus, Info } from 'lucide-react';
import { useResidentialQuoteCountMetrics } from '../../../../hooks/jobber/residential';
...
export function QuoteOptionsAnalysis({ filters }: QuoteOptionsAnalysisProps) {
  const { data: quoteCountMetrics, isLoading } = useResidentialQuoteCountMetrics(filters);
              ...
              {QUOTE_COUNT_BUCKET_ORDER.map((bucket) => {
                ...
                return metric ? (
                  ...
                  <tr key={bucket} className="text-gray-400">
                    ...
                    <td colSpan={5} className="py-3 px-4 text-center">No data</td>
                  </tr>
                );
              })}
            </tbody>
