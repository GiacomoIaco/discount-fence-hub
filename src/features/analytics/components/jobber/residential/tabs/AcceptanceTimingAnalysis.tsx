...
// Shows distribution of days from quote to acceptance for mature quotes (Jan-June 2025)
...
// Fetch won opportunities from Jan-June 2025 with days_to_decision
function useAcceptanceTimingData() {
  return useQuery({
    ...
    queryFn: async () => {
      const { data, error } = await supabase
        ...
        .lte('first_quote_date', '2025-06-30')
    ...
    staleTime: 10 * 60 * 1000, // 10 minutes - this is static historical data
  });
}
...
export function AcceptanceTimingAnalysis() {
  const { data: rawData, isLoading, error } = useAcceptanceTimingData();
  ...
  if (!analysis) {
    return (
      <div className="bg-gray-50 border border-gray-200 rounded-lg p-6 text-center text-gray-500">
        No won opportunities found for Jan-June 2025 with decision timing data.
      </div>
    );
  }
    ...
    <div className="space-y-6">
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
        ...
        <p className="text-sm text-blue-700">
          Based on <strong>{analysis.total} won opportunities</strong> from January - June 2025.
