 ...
 *
 * Note: FenceCalculator now includes nails and concrete automatically with
 * built-in fallback materials. All quantities are RAW (unrounded).
 * SKU-level costing applies Math.ceil() when summing costs.
 */
...
/**
 * Calculate material costs for a Wood Vertical SKU
...
export function calculateWoodVerticalMaterialCost(
  ...
  // Calculator includes nails and concrete with built-in defaults
  const result = calculator.calculateWoodVertical(
    product,
    SKU_STANDARD_ASSUMPTIONS,
    [] // No labor rates needed for material cost
  );

  // SKU-level: Apply Math.ceil() to each material quantity
...
/**
 * Calculate material costs for a Wood Horizontal SKU
 * FenceCalculator now includes nails and concrete automatically
 */
...
/**
 * Calculate material costs for an Iron SKU
 * FenceCalculator now includes concrete automatically (iron uses screws, not nails)
 */
export function calculateIronMaterialCost(
  ...
  // Calculator includes concrete with built-in defaults
